...
export const verifyTokenGenerator = () => {
  let randomCode;

  do {
    randomCode = Math.ceil(Math.random() * 10000);
  } while (randomCode > 9999 || randomCode < 1000);
  return randomCode;
};

// Takes data and turns that to JWT and returns it
export const tokenGenerator = (data) =>
  JWT.sign(data, process.env.JWT_SECRET, { expiresIn: "3h" });
...
export const UnauthorizedResponse = (res) =>
  res
    .status(401)
    .json({ message: "You must authenticate to get the requested response" });

export const NotFoundResponse = (res, err = {}) =>
  res.status(404).json({ err, message: "There is no data for this ID" });
